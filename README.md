...
- 「跟它细聊」是次要入口，有三个已知限制（融合审查确认，留待）：
  ① 激活只影响**新建**会话，SPA 若恢复/复用上一个会话，细聊到的可能是上一个助手；
  ② 细聊会话的 cwd 不是应用 workspace，资料与工作手册在细聊里看不见——「同一个
  助手」只在 preset（人格 + 工具集）层成立；
  ③ 把配置文件里的 key 物化进环境变量后，细聊界面里 DSH 自己的 Models 页会被锁死
  （环境变量层在 DSH 的凭证解析里优先级最高）——用万象自己的「模型设置」即可，
  不受影响。
