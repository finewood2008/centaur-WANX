...
**它认得出端口上的旧实例。** 探测到端口上跑着旧版万象时**拒绝复用**，并在窗口里给一个「收掉旧的，用新版打开」的按钮——只对 `cmdline` 能确认是万象服务的进程动手，认不出的一律不碰。

判断旧不旧有两条：`/health` 的接口版本 `ui`，以及**服务启动时刻和源码最新修改时间的比较**。后一条是必须的——`index.html` 是每次请求现读磁盘的，旧进程会把**新的 HTML** 配上**旧的路由表**发出去，界面于是半新半旧（加 `.png` 支持之前就出过这个：页面引用了 logo，进程却不认识 `.png`，左上角一张裂图）。
